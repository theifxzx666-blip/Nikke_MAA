...
| `supports/` | 依赖与支持组件；`shizuku/` 随仓库提交，`maacore_sdk/` 和 `prebuilt/` 是本机缓存 |
...
本机调试所需的 MaaCore SDK 与预编译 native 库统一放在 `supports/`；APK、后端 jar、截图和日志放在 `outputs/android_probe/`。本机缓存和构建产物不提交到 GitHub。
